...
SBATCH -e error_file_experiments.e
SBATCH -o output_file_experiments.o
...
start=`date +%s`
for i in /cfs/klemming/nobackup/e/erikzhi/ENCODE/experiments/*.bam;
do
    echo "Processing sample $i"
    sample=`basename ${i}`
    out_dir=$'/cfs/klemming/nobackup/e/erikzhi/ENCODE/salmon_output/experiments/'
    STARTTIME=$(date +%s)
