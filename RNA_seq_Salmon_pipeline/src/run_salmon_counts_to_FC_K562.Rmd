...
ctrl_files2exp <- read.table("/scratch/erikzhi/ENCODE_data/ENCODE_controls_files2experiments.csv", header = FALSE, sep=',')
pert_files2exp <- read.table("/scratch/erikzhi/ENCODE_data/ENCODE_perturbations_files2experiments.csv", header = FALSE, sep=',')
ctrl_meta <- read.table("/scratch/erikzhi/ENCODE_data/ENCODE_controls_meta.tsv", skip = 1, header = TRUE, sep='\t')
pert_meta <- read.table("/scratch/erikzhi/ENCODE_data/ENCODE_perturbations_meta.tsv", skip=1, header = TRUE, sep='\t')
txdb_from_gff <-makeTxDbFromGFF("/scratch/erikzhi/ENCODE_data/ref/gencode.v37.annotation.gtf")
keys_txdb_from_gff <- keys(txdb_from_gff, keytype = "TXNAME")
gencode_v37 <- select(txdb_from_gff, keys_txdb_from_gff, "GENEID", "TXNAME")
...
```{r pressure, echo=FALSE, message=FALSE, results=FALSE}
setwd("/scratch/erikzhi/ENCODE_data")

#aggregate TPMs from different experiments into one table
final_table_tpm_ctrls <- handle_tpm_salmon_output('/scratch/erikzhi/ENCODE_data/salmon_output/controls', unique(target_ctrl_files), target_genes, gencode_v37)
final_table_tpm_perts <- handle_tpm_salmon_output('/scratch/erikzhi/ENCODE_data/salmon_output/experiments', unique(target_pert_files), target_genes, gencode_v37)
...
write.table(final_Y, file='/scratch/erikzhi/ENCODE_data/tables/ymatrix_k562.csv', quote=FALSE, sep='\t', col.names = TRUE, row.names = TRUE)
write.table(final_P, file='/scratch/erikzhi/ENCODE_data/tables/pmatrix_k562.csv', quote=FALSE, sep='\t', col.names = TRUE, row.names = TRUE)
```
